...
void MlNdt::data_cb(const nav_msgs::Odometry::ConstPtr & odom, const sensor_msgs::LaserScan::ConstPtr & laser){
  ...
  tf_list_.waitForTransform(robot_base_frame_, laser->header.frame_id,laser->header.stamp, ros::Duration(5.0));
  pcl_ros::transformPointCloud(robot_base_frame_, laser_msg, laser_base, tf_list_);
  pcl::fromROSMsg(laser_base,laser_base_points);
  //std::cout<<laser_base_points.getMatrixXfMap()<<std::endl;
  
  tf::Pose pose_tf;
  Eigen::Vector3f pose;
  tf_list_.waitForTransform(world_farme_,odom->header.frame_id, odom->header.stamp, ros::Duration(5.0));

  tf::poseMsgToTF(odom->pose.pose,pose_tf);
  pose<<pose_tf.getOrigin().getX(),pose_tf.getOrigin().getY(),tf::getYaw(odom->pose.pose.orientation);
