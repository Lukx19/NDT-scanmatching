...
Layer::point_t Layer::transformPoint(const Id_t id,const pose_t & transform)const{
  return std::move(transformPoint(points_->at(id),transform));
}

Layer::point_t Layer::transformPoint(const point_t & point,const pose_t & transform)const{
...
Eigen::Matrix3f Layer::makeToSPD(Eigen::Matrix3f & mat){
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3f> saes(mat,Eigen::EigenvaluesOnly);
  float k =0;
...
std::pair<size_t, size_t> Layer::getFieldCoordintes(const point_t & point)const{
  ...
  size_t fieldx = static_cast<size_t>(std::floor((point(0)+max_range_)/resolution));
  // flips space of points in y axis from [0,range] to [range,0] 
  size_t fieldy = static_cast<size_t>(std::floor((max_range_- point(1))/resolution));
...
bool Layer::calculateNdt(pose_t & transf, points_t & points){
  ...
  for(size_t iter=0;iter<max_iteration;++iter){
    ...
    for(auto point:points){
      ...
      std::pair<size_t, size_t> new_coords = getFieldCoordintes(transform_point);
      
      Eigen::Matrix2f inv_variace =  fields_[new_coords.first][new_coords.second].calcVariance().inverse();
      point_t diffrence = transform_point - fields_[new_coords.first][new_coords.second].calcMean();
      float score_part=expf((-diffrence.transpose()*inv_variace*diffrence)(0)/2);
      // prepare for calculating H * delta_p = -g where delta_p is change in transformation
      // incrementaly summing parts of  g for each point
      jacobian<<1,0,-point(0)*si-point(1)*co,
                0,1,point(0)*co-point(1)*si;
      for(long r=0;r<3;++r){
        g(r) += (diffrence.transpose()*inv_variace*jacobian.col(r))(0)*score_part;  
      }

      // create hessian
      for(long r=0;r<3;++r){
        for(long c=0;c<3;++c){
          float jacc_r = (diffrence.transpose()*inv_variace*jacobian.col(r))(0);
          float jacc_c = (diffrence.transpose()*inv_variace*jacobian.col(c))(0);
          point_t hess;
          if(r == 2 && c == 2)
            hess<<-point(0)*co+point(1)*si , -point(0)*si - point(1)*co;
          else
            hess<<0,0;
          hessian(r,c) += score_part*
                          (jacc_r*jacc_c + 
                            diffrence.transpose()*inv_variace*hess +
    ...
    if(ldtl.info() != Eigen::Success)
      throw std::logic_error("Error in computation of diffrence in transformation");
    p+=delta_p;
  }
